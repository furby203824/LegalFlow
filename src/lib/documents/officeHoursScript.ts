...
export function generateOfficeHoursScript(data: CaseData): string {
  ...
  for (const offense of data.offenses) {
    lines.push(
      `  Charge ${offense.letter}: "In that you did, ${offense.onOrAbout !== false ? "on or about " : ""}${fmtFull(offense.offenseDate)},${offense.offenseTime ? ` at approximately ${offense.offenseTime},` : ""}`
    );
