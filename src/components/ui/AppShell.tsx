...
import {
  ...
  Users, Settings, LogOut, Scale, Shield,
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-64 flex flex-col bg-primary text-white transition-transform duration-200 ease-in-out lg:hidden",
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Topbar */}
        <header className="h-16 bg-bg border-b border-border flex items-center justify-between px-4 sm:px-6">
          {/* Mobile hamburger */}
          <button
            onClick={() => setMobileOpen(true)}
            className="lg:hidden p-2 -ml-2 text-neutral-mid hover:text-neutral-dark"
          >
            <Menu size={22} />
          </button>
          <div className="hidden lg:block text-sm text-neutral-mid">
        ...
        {/* Footer */}
        <footer className="h-10 bg-neutral-light border-t border-border flex items-center justify-between px-4 sm:px-6 text-xs text-neutral-mid">
